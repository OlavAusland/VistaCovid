            ...
            querySnapshot.forEach(async (doc) => {
                ...
                if (DetectDanger(10, 100, room.heartRate))
                    sendPushNotification({
                        to: expoPushToken, sound: 'default', title: `${room.id}: 
            Heart Rate = ${room.heartRate?.length > 0 ? room.heartRate[room.heartRate.length - 1].value : ''}`
                    }).then(() => console.log('sent'));
                if (DetectDanger(10, 100, room.respirationRate))
                    sendPushNotification({
                        to: expoPushToken, sound: 'default', title: `${room.id}:  
            Respiration Rate = ${room.respirationRate?.length > 0 ? room.respirationRate[room.respirationRate.length - 1].value : ''}`
                    }).then(() => console.log('sent'));
                if (DetectDanger(10, 100, room.oxygenLevel))
                    sendPushNotification({
                        to: expoPushToken, sound: 'default', title: `${room.id}:\n
            Oxygen Level = ${room.oxygenLevel?.length > 0 ? room.oxygenLevel[room.oxygenLevel.length - 1].value : ''}`
                    }).then(() => console.log('sent'));
            });
        });
    }, []);
